...
public class GameActivity extends Activity implements RecognitionListener {
	...
	public void onCreate(Bundle state) {
		...
		if (gameMode.equals(Constants.GAMEMODE_EASY)) {
			bmp = Bitmap.createScaledBitmap(BitmapFactory.decodeResource(
					getResources(), R.drawable.game_easy_background), size.x,
					size.y, true);
		} else if (gameMode.equals(Constants.GAMEMODE_NORMAL)) {
			bmp = Bitmap.createScaledBitmap(BitmapFactory.decodeResource(
					getResources(), R.drawable.game_normal_background), size.x,
					size.y, true);
		} else {
			bmp = Bitmap.createScaledBitmap(BitmapFactory.decodeResource(
					getResources(), R.drawable.game_hard_background), size.x,
					size.y, true);
		}
		...
		lstCorrect = new ArrayList<>();
		lstWrong = new ArrayList<>();
		lstThreshold = new ArrayList<>();
		lstImage = new ArrayList<>();
		lstPhoneme = new ArrayList<>();
	...
	private void game() {
		...
		if (iteration == Constants.itemsPerGame) {
			...
			// set option 1
			textViewOption1.setText(randBool ? lstCorrect.get(index) : lstWrong
					.get(index));

			// set option 2
			textViewOption2.setText(randBool ? lstWrong.get(index) : lstCorrect
					.get(index));
